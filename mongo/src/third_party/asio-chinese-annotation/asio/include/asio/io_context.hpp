...
/*
Boost.Asio代码风格。Asio为了可读性将部分较复杂的类的声明和实现分成了两个头文件，在声明的
头文件末尾include负责实现的头文件。impl文件夹包含这些实现的头文件。另外，还有一个常见的
关键词是detail，不同操作系统下的各种具体的代码会放在detail文件夹下。
*/
...
/*
namespace detail {
#if defined(ASIO_HAS_IOCP) //windows平台 
...
//class service_base: public asio::io_context::service     service_base继承该类
