...
void Lock::GlobalLock::_enqueue(LockMode lockMode, unsigned timeoutMs) {
	...
	//log() << "ddd test.... global lock, lock mode:" << modeName(lockMode);
...
Lock::DBLock::DBLock(OperationContext* opCtx, StringData db, LockMode mode)
	...
	//LockerImpl<>::lock
	//对该库加mode锁
	//log() << "ddd test.... DB lock, db: " << db.toString()<< " lock mode:" << modeName(mode);
...
void Lock::DBLock::relockWithMode(LockMode newMode) {
	...
	//log() << "ddd test..relockWithMode.. DB lock,  lock mode:" << modeName(newMode);
...
Lock::CollectionLock::CollectionLock(Locker* lockState, StringData ns, LockMode mode)
	...
	//log() << "ddd test.... collection lock, ns: " << ns.toString()<< " lock mode:" << modeName(mode);
...
void Lock::CollectionLock::relockAsDatabaseExclusive(Lock::DBLock& dbLock) {
    ...
    //log() << "ddd test..relockAsDatabaseExclusive.. lock,  lock mode:" << modeName(MODE_X);
...
void Lock::ResourceLock::lock(LockMode mode) {
	...
	//LockerImpl::lock
	//log() << "ddd test.... ResourceLock lock, " << " lock mode:" << modeName(mode);
